...
- Canvas 绘制海拔图
- 距离 / 海拔刻度
- **交互联动**：鼠标悬浮显示十字线 + 距离/海拔/坡度提示
- **点击跳转**：点击海拔图某点，地图自动定位到对应位置
- **🔄 校准海拔**：使用 DEM API 补全缺失海拔数据（上传时自动检测）
- **📐 偏移校正**：GCJ-02 → WGS-84 坐标转换（佳明等国内设备自动识别）
...
### 🔗 路书链接
- 上传 GPX 时自动识别 Strava / iGPSPORT / Garmin 链接（从 `<metadata><link>` 提取）
- Garmin 文件名匹配 `COURSE_\d+` 时自动生成链接
- 手动输入框可补充或覆盖
...
13. **海拔校准**：点击海拔剖面「🔄 校准海拔」补全缺失海拔
14. **偏移校正**：点击「📐 偏移校正」切换 GCJ-02/WGS-84 坐标
15. **右键菜单**：在地图上右键可快速放置标记、测距、复制坐标
